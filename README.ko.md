...
3. Gatekeeper 메타데이터 해제 (서명 안 된 빌드):
...
> **"손상된 파일" 에러?** macOS Sequoia+ 가 다운로드 앱에 추가 격리 메타데이터를
> 붙여서 뜨는 거예요. 위 두 `sudo xattr` 명령이 그걸 지워줍니다 — 이게 앱과
> 여러분 사이의 유일한 벽이에요. Apple Developer ID 서명은 로드맵에 있습니다.

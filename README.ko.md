...
- **PPTX / HWP / HWPX** 파싱 — kordoc + 사이드카 기반
- **스캔 PDF OCR** — 이미지 기반 PDF에서 텍스트 추출
- **RAG 자동 주입** — Knowledge Base 문서가 채팅 컨텍스트에 자동 검색·삽입

### 🧬 LoRA 파인튜닝

- 전체 파이프라인: 데이터셋 생성 → 학습 → 어댑터 머지
- `mlx-lm` LoRA 지원 (rank, layers, epochs 설정 가능)
- 어댑터 머지(fuse)로 독립 모델 생성

### 🔀 모델 블렌딩

- 같은 아키텍처 모델 2개를 **SLERP / Linear / TIES / DARE** 방식으로 합성
- 진행률 표시 + 인터랙티브 GUI

### 🏓 핑퐁 — 대폭 강화

- **파일 / 코드 / URL 첨부** — 토론에 문서와 코드 첨부 가능
- **편집기 스타일 코드블록** — 다크 배경, 라인넘버, 언어 헤더
- **Thinking 자동 제거** — 모델 사고과정 숨기고 답변만 표시
- **긴 응답 자동 분할** — 300자 기준 여러 말풍선으로 나눠 표시
- **"입력중..." 인디케이터** — 모델 추론 중 로딩 상태 표시 (i18n)
- **Topic 라벨** — 사용자 메시지 "나" 대신 "주제"로 표시

### 🔧 개선사항

- **웹 검색** — 사용자가 명시적으로 요청할 때만 호출
- **설정 UI** — 카드 섹션으로 그룹핑
- **파일 추출 한도** — 80 KB → 200 KB 확대
- **Mac 모델명** — M4 Max 등 칩 이름 하드웨어 스펙 카드에 표시
